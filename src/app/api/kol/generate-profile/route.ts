...
export async function POST(req: Request) {
    ...
    const rateCheck = checkRateLimit(`kol:${clientIp}`, RATE_LIMITS.kolGeneration);
    ...
    try {
        ...
        try {
            ...
            // Return raw text if parsing fails, but for profile generation we expect JSON
            console.warn("Failed to parse JSON response:", parseError);
            return NextResponse.json({ result: text });
        }

    } catch (error) {
        console.error("KOL Profile Generation API error:", error);
        return NextResponse.json(

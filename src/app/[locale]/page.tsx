...
import { Camera, Sparkles, ArrowRight, Zap, Shield, Clock, Film, Users, Wand2, Mic, Video, Bug, ArrowUpRight, Code, ShieldCheck, FileText } from 'lucide-react';
...
export default function HomePage() {
  ...
  const tHub = useTranslations('CreatorHub');
  ...
  return (
    <main className="w-full">
      ...
      <section aria-label="Hero" className="relative py-32 px-6 overflow-hidden min-h-[90vh] flex items-center">
        ...
        <div className="relative max-w-6xl w-full mx-auto text-center space-y-12">
          {/* Badge */}
          ...
          <m.div
            ...
            className="flex flex-wrap items-center justify-center gap-2 mb-12 pt-8"
          >
            {categories.map((cat) => (
              <button
                key={cat.id}
          ...
          <m.div
            layout
            className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 auto-rows-[220px] max-w-6xl mx-auto w-full text-left"
          >
            <AnimatePresence mode="popLayout">
              {filteredTools.map((tool, index) => {
                const badgeStr = tHub(`tools.${tool.key}.badge` as any);
